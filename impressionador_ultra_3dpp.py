...
Atualização focada no que você pediu:
✅ Tunnel com "motion blur" fake (persistência temporal)
✅ Terrain com "shading por normal" (luz/sombra no relevo)

Mantém a essência + extras anteriores:
  1) Donut 3D girando
  2) Chuva Matrix
  ...
  7) Metaballs
  8) Game of Life
  9) Túnel Infinito (agora com Motion Blur)
 10) Wireframe Rotating Cube
 11) Terrain Wave (agora com Normal Shading)

Controles (em qualquer cena):
  1..11 -> troca de cena (1..9 direto; J=10 e K=11 durante execução)
...
class Theme:
    ...
    def fx(self, k: float, t: float, base_h: float = 0.6) -> str:
        ...
        k = clamp(k)
        if self.name == 'Normal':
            r,g,b = hsv_to_rgb(base_h + 0.12*k, 0.55, 0.18 + 0.82*k)
            return fg_rgb(r,g,b)
        if self.name == 'Suave':
...
class Tunnel(Scene):
    key = '9'
    name = 'Túnel Infinito (Motion Blur)'
    ...
    def step(self, vw, vh, t, dt, speed, theme: Theme):
        ...
        # phase speed controls perceived forward motion
        self.phase += dt * 1.0 * speed

        # Motion blur persistence:
        # decay close to 1 => longer trails. Increase slightly with speed.
        ...
        for y in range(vh):
            ...
            for x in range(vw):
                ...
                if mono:
                    ...
                    # color by depth: more depth -> brighter
                    kk = clamp((z/6.0))
                    # mix with val to accent trails
...
class Terrain(Scene):
    key = '11'
    name = 'Terrain Wave (Normal Shading)'
    ...
    def step(self, vw, vh, t, dt, speed, theme: Theme):
        ...
        grid = [[' ' for _ in range(vw)] for _ in range(vh)]
        ibuf = [[0.0 for _ in range(vw)] for _ in range(vh)]  # intensity buffer
        zbuf = [[-1e9 for _ in range(vw)] for _ in range(vh)]
        ...
        # render far -> near or near -> far? We'll keep near->far but zbuf prevents overdraw.
        for zi in range(1, 115):
            ...
            for xi in range(vw):
                ...
                # intensity combines depth + lighting
                inten = clamp(shade * (0.30 + 0.70*k_depth))

                # choose char by intensity (more smooth than pure depth)
                if inten > 0.80:
                    ch = '█'
                elif inten > 0.60:
                    ch = '▓'
...
HELP_TEXT = [
    "Controles:",
    "  1..11 troca de cena", 
    "  (durante execução: 1..9 direto | J=10 Cubo | K=11 Terreno)",
    "  T     troca tema (Normal/Suave/Neon/Psicodélico/Mono)",
    "  C     cor ON/OFF", 
    "  +/-   velocidade", 
    "  P     pausa", 
    ...
    "",
    "Novidades:",
    "  - Cena 9: Motion Blur (persistência temporal)",
    "  - Cena 11: Normal Shading (luz/sombra no relevo)",
    "",
...
def overlay_help(content: list[str], vw: int, vh: int, theme: Theme) -> list[str]:
    box_w = min(vw-4, max(46, max(len(s) for s in HELP_TEXT) + 4))
...
def menu(theme: Theme):
    ...
    subtitle = 'Essência + extras + 3D fake (com Motion Blur e Normal Shading)'
...
def run_scene(scene_idx: int, theme: Theme) -> int:
    ...
    try:
        while True:
            ...
            if k:
                kk = k.lower()
                if kk in KEY_TO_SCENE and kk not in ('10','11'):
